...
// --- CONFIGURAÇÃO DE ENV (LOCAL + GLOBAL) ---
...
// Se não achou a chave, tenta global
...
if (!API_KEY) {
  console.error(`\n\x1b[31m[ERRO CRÍTICO] API Key não encontrada.\x1b[0m`);
  console.error(
...
// Lista atualizada com modelos REAIS e EXISTENTES
const MODEL_OPTIONS = [
  "gemini-2.5-flash",
  "gemini-2.5-pro",
...
// #region 2. Lógica da IA (Smart Generate + Failover)
// =============================================================================
    ...
    try {
      if (modelName !== MODEL_OPTIONS[0]) {
        process.stdout.write(
          `\n\x1b[90m(Failover: tentando ${modelName}...)\x1b[0m `
        );
      }
...
async function init() {
  ...
  if (args.includes("commit")) {
    ...
    if (workIndex !== -1 && args[workIndex + 1]) {
      ...
      console.log(
        `\x1b[35m[WORK MODE] Tarefa detectada: AB#${workTaskId}\x1b[0m`
      );
    }
    ...
    try {
      ...
      if (!analise.includes("[APROVADO]")) {
        ...
        await new Promise((resolve) => {
          rl.question(
            "\x1b[33mDeseja IGNORAR o alerta e prosseguir? (s/n): \x1b[0m",
            (resp) => {
  ...
  if (!process.stdin.isTTY) {
    // console.log(`\x1b[34m[SISTEMA] Lendo Pipe...\x1b[0m`); // Comentado para não poluir saída se não houver pipe real
    const pipedData = await readStdin();
    if (pipedData && pipedData.trim()) {
  ...
  const perguntaInicial = args
    .filter(
      (arg, index) =>
        !arg.startsWith("--") &&
        index !== imgFlagIndex &&
        index !== imgFlagIndex + 1
    )
    .join(" ");
